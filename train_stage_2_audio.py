...
from src.dataset.dance_video_audio import HumanDanceVideoDataset
...
class Net(nn.Module):
    ...
    def forward(
        ...
        audio_tensor = tgt_audio_full.to(device='cuda')
        # print("Audio Guider Input: ", audio_tensor.shape)
        # audio_fea = self.audio_guider(audio_tensor.unsqueeze(0)) 
...
def main(cfg):
    ...
    #     strict=False,
    # )
    audio_guider.load_state_dict(
        torch.load(
            os.path.join(stage1_ckpt_dir, f"audio_guider-{stage1_ckpt_step}.pth"),
            map_location="cpu",
        ),
        strict=False,
    )
    
    print("Succeeded in loading in trained audio guider")
    ...
    audio_guider.requires_grad_(False)
    ...
    # MAKE SURE ITS DOING THE RIGHT THING HERE
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(net):
                ...
                # TODO - figure out what batch looks like
                tgt_audio_full = batch["tgt_audio_full"]
                # print("TGT AUDIO FULL DIMS: ", tgt_audio_full.size())
    ...
    plt.savefig('loss_plot_stage_2.png')
...
def save_checkpoint(model, save_dir, prefix, ckpt_num, total_limit=None):
    ...
    for key in state_dict:
        if "motion_module" in key:   # TODO - check if motion module makes sense
