...
class Net(nn.Module):
    def __init__(
        ...
        pose_guider: PoseGuider, # TODO - modify architecture for audio gudier
    ...
    def forward(
        ...
        ref_image_latents,
        clip_image_embeds,
        pose_img, # TODO - modify to pass in audio instead
        uncond_fwd: bool = False,
    ):
        img_to_show = pose_img.squeeze().cpu().permute(1, 2, 0).numpy()
        print(img_to_show.shape)
        # plt.imshow(pose_img.squeeze().cpu().numpy())
        plt.imshow(img_to_show)
        plt.savefig('output_image.png', bbox_inches='tight', pad_inches=0)
        # plt.show()
        pose_cond_tensor = pose_img.to(device="cuda")
        print("Pose Guider Input: ", pose_cond_tensor.shape)
        pose_fea = self.pose_guider(pose_cond_tensor) # TODO - modify forward pass to have audio passed into audio_guider
...
def log_validation(
    ...
    # FIXING GITHUB VALIDATION ISSUE TODO - understand it
    # reference_unet = copy.deepcopy(ori_net.reference_unet)
    # denoising_unet = copy.deepcopy(ori_net.denoising_unet)
    pose_guider = ori_net.pose_guider # TODO - extract audio_guider from net
...
def main(cfg):
    ...
    if cfg.pose_guider_pretrain:
        ...
        pose_guider = PoseGuider(  # replace with Audio Guider
    ...
    net = Net(
        reference_unet,
        denoising_unet,
        pose_guider,  # TODO: replace with Audio Guider here
        reference_control_writer,
        reference_control_reader,
    )
    ...
    train_dataset = HumanDanceDataset( # TODO - replace with our own dataset, or just get audio from human dance dataset
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader): 
            ...
            with accelerator.accumulate(net):
                ...
                for name, param in net.pose_guider.named_parameters():
            ...
            if accelerator.sync_gradients:
                ...
                #         # reference_control_reader.clear()
                #         # reference_control_writer.clear()
                #         # reference_control_writer.do_classifier_free_guidance = False
                #         # reference_control_reader.do_classifier_free_guidance = False
